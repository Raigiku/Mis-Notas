...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C02361();
            ...
            class C02361 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem next = C03901.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02371();
            ...
            class C02371 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem next = C03912.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C03912.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C03912.this.mQueue.removeMessages(2);
                                    C03912.this.mQueue.removeMessages(3);
                        ...
                        C03912.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
