...
public class Explode extends Visibility {
    ...
    public Animator onDisappear(ViewGroup viewGroup, View view, TransitionValues transitionValues, TransitionValues transitionValues2) {
        ...
        float translationX = view.getTranslationX();
        float translationY = view.getTranslationY();
        int[] iArr = (int[]) transitionValues.view.getTag(C0115R.id.transition_position);
        if (iArr != null) {
        ...
        calculateOut(viewGroup, rect, this.mTempLoc);
        return TranslationAnimationCreator.createAnimation(view, transitionValues, i, i2, translationX, translationY, f + ((float) this.mTempLoc[0]), f2 + ((float) this.mTempLoc[1]), sAccelerate);
    }
    ...
    private void calculateOut(View view, Rect rect, int[] iArr) {
        ...
        centerX2 /= calculateDistance;
        centerY /= calculateDistance;
        float calculateMaxDistance = calculateMaxDistance(view2, width - i, height - i2);
        iArr[0] = Math.round(centerX2 * calculateMaxDistance);
        iArr[1] = Math.round(calculateMaxDistance * centerY);
    }
