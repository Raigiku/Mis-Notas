...
public final class ColorUtils {
    ...
    public static Color compositeColors(@NonNull Color color, @NonNull Color color2) {
        if (Objects.equals(color.getModel(), color2.getModel())) {
            ...
            color = color.alpha();
            float alpha = color2.alpha() * (1.0f - color);
            int componentCount = color2.getComponentCount() - 1;
            components2[componentCount] = color + alpha;
            if (components2[componentCount] > 0.0f) {
                color /= components2[componentCount];
                alpha /= components2[componentCount];
            }
            for (int i = 0; i < componentCount; i++) {
                components2[i] = (components[i] * color) + (components2[i] * alpha);
            }
