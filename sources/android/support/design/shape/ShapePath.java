...
public class ShapePath {
    ...
    public static class PathLineOperation extends PathOperation {
        ...
        public void applyToPath(Matrix matrix, Path path) {
            ...
            matrix.invert(matrix2);
            path.transform(matrix2);
            path.lineTo(this.f15x, this.f16y);
            path.transform(matrix);
        }
    }
    ...
    public void lineTo(float f, float f2) {
        ...
        pathLineOperation.f15x = f;
        pathLineOperation.f16y = f2;
    ...
    public void addArc(float f, float f2, float f3, float f4, float f5, float f6) {
        ...
        f5 = (double) (f5 + f6);
        this.endX = ((f + f3) * 0.5f) + (((f3 - f) / 2.0f) * ((float) Math.cos(Math.toRadians(f5))));
        this.endY = ((f2 + f4) * 0.5f) + (((f4 - f2) / 2.0f) * ((float) Math.sin(Math.toRadians(f5))));
    }
